...
class ThemeManager {
    ...
    private pendingWorkerRequests = new Map<number, { resolve: (color: number) => void; reject: (error: Error) => void }>();
     ...
     */
    generateTheme(sourceColor: number, isDark: boolean = false): ThemeColors {
     ...
     */
    private loadThemeFromSession(): ThemeColors | null {
        try {
            const storedTheme = sessionStorage.getItem(THEME_STORAGE_KEY);
            if (storedTheme) {
                return JSON.parse(storedTheme) as ThemeColors;
            }
        } catch (error) {
     ...
     */
    private saveThemeToSession(theme: ThemeColors): void {
        try {
    ...
    public getCurrentTheme(): ThemeColors {
        ...
        // 3. 使用默认主题色
        globalThemeState.currentTheme = DEFAULT_THEME_COLORS;
     ...
     */
    async updateThemeFromImage(imageUrl: string, isDark: boolean = false): Promise<number | undefined> {
